...
void initializeLCD(){
	...
	char commands[] = { 0b0010, 
						...
						0b00000001};
	delay_s(1);
	for(i = 0; i < 4; i++){ // 4 = tamanho commands
...
void sendLCDText(char text[]){
	...
	for(i = 0; i < len; i++)sendDataLCD(text[i]);
}
...
void interruptTreatment(){
	...
	if(INTCON3bits.INT2IF){
		motorRotation++; //Pistão subiu e desceu.
		INTCON3bits.INT2IF = 0;
	}	
	if(INTCONbits.TMR0IF){
		...
		calculateSpeedRPM(motorRotation, timer); //Recebe quantas vezes o pistão subiu e desceu
										  //e calcula a RPM.
		odometer = 0; // Zera o Odometro para a próxima volta.
        motorRotation = 0; //Zera o contador de rotação do motor.
		timer = 0; // Zera o Timer para a próxima interrupção
		INTCONbits.TMR0IF = 0;	
	}
}
